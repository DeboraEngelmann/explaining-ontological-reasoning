...
defeasible_inference(Result,Arg,Goal) :- 
		not .list(Goal) & .list(def_rule(defeasible_rule(Goal,_)[as(_)]))
		& def_rule(defeasible_rule(Goal,Condition)[as(AS)]) & .print("Lista! 1- Goal: ", Goal, "Condition: ", Condition) & concat(Arg,[defeasible_rule(Goal,Condition)[as(AS)]],X)
...
defeasible_inference(Result,Arg,Goal) :- 
		not .list(Goal) 
		& def_rule(defeasible_rule(Goal,Condition)[as(AS)]) & .print("1- Goal: ", Goal, "Condition: ", Condition) & concat(Arg,[defeasible_rule(Goal,Condition)[as(AS)]],X)
		...
		// & .print("1-CQs terms: ", CqsS) & .term2string(CqsS,Cqs)
		// & answer(CqsS). // answer all them
		
defeasible_inference(Result,Arg,Goal) :- 
		not .list(Goal) & def_rule(defeasible_rule(Goal,Condition)[as(AS)]) & .print("2- Goal: ", Goal, "Condition: ", Condition) & concat(Arg,[defeasible_rule(Goal,Condition)[as(AS)]],X) // & .print(Condition)
		...
		// & .print("2- ", CqsS) & .term2string(CqsS,Cqs) 
		// & answer(CqsS). // answer all them
...
defeasible_inference(Goal) :- 
		not .list(Goal) & def_rule(defeasible_rule(Goal,Condition)[as(AS)]) & .print("3- Goal: ", Goal, "Condition: ", Condition) 
		& defeasible_inference(Condition) 
		& not(rebutted(defeasible_rule(Goal,Condition))) 
		& not(undercut(defeasible_rule(Goal,Condition))).
		// & jia.get_unifier(defeasible_rule(Goal,Condition)[as(AS)],Unifier) //& .print("Unifier: ", Unifier) // get the unifier
		// & jia.get_cqs(AS,Unifier,Cqs) //& .print(Cqs)// get the critical questions instantiated accordign unifier
		// & .print("3- ", CqsS) & .term2string(CqsS,Cqs) 
		// & answer(CqsS). // answer all them
		
defeasible_inference(Goal) :- 
		not .list(Goal) & def_rule(defeasible_rule(Goal,Condition)[as(AS)]) & .print("4- Goal: ", Goal, "Condition: ", Condition) 
		& not(contrary(Goal,Contrary1) & strict_inference(Contrary1)) // If defeater preemption is enabled, then  a goal is defeasibly derivable if it is 
		& defeasible_inference(Condition) 
		& not(contrary(Goal,Contrary2) 
			& strict_rule(Contrary2,Condition2) & Condition2 \== true 
		...
		// & jia.get_unifier(defeasible_rule(Goal,Condition)[as(AS)],Unifier) //& .print("Unifier: ", Unifier) // get the unifier
		// & jia.get_cqs(AS,Unifier,Cqs) //& .print(Cqs)// get the critical questions instantiated accordign unifier
		// & .print("4- ", CqsS) & .term2string(CqsS,Cqs) 
		// & answer(CqsS). // answer all them
...
// & jia.get_unifier(defeasible_rule(Goal,Condition)[as(AS)],Unifier) //& .print("Unifier: ", Unifier) // get the unifier
// & jia.get_cqs(AS,Unifier,Cqs) //& .print(Cqs)// get the critical questions instantiated accordign unifier
// & .print("9- ", CqsS) & .term2string(CqsS,Cqs) 
// & answer(CqsS). // answer all them
